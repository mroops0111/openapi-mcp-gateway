...
class PassthroughFlowHandler(OAuthFlowHandler):
    ...
    Suitable when the gateway and the upstream API share an OAuth realm
    so the MCP client's existing token is already accepted upstream.
    Typically used by FastAPI integrations where the gateway is mounted
    onto the same app it exposes.
    """
