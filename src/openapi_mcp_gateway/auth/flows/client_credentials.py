...
class ClientCredentialsFlowHandler(OAuthFlowHandler):
    ...
    fetch a single upstream access token shared across all MCP clients. The
    resulting setup carries only an ``AuthResolver`` plus an ``on_shutdown``
    hook that closes the underlying token source's HTTP client.
    """

    def build(self, flow_context: OAuthFlowContext) -> OAuthFlowSetup:
        """Construct ``ClientCredentialsTokenSource`` and the matching resolver."""
